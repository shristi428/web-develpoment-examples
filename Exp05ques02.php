...
Input for file one:-<input type="text" name="text1" placeholder="Text"></br>
Input fot file two:-<input type="text" name="text2" placeholder= "TEXT"></br>
<input type="submit" name="submit"></br>
 ...
 $t2=$_GET["text2"];
		...
		$fp=fopen("abc.txt",'a');
		fwrite($fp,"$t1\r\n");
    echo"Output of file one<br>";
	echo file_get_contents("abc.txt")."<br>";
	
		$fp2=fopen("xyz.txt",'a');
		fwrite($fp2,"$t2\r\n");
		echo"Output for file two<br>";
	echo file_get_contents("xyz.txt")."<br>";
	
	if(copy("abc.txt","xyz.txt")==true){
		echo"file copied successfully<br>";
		echo"Data in the file is <br>";
		echo file_get_contents("xyz.txt")."<br>";
	}
	else{
		echo"file dose not copied successfully<br>";
	}
